...
SKYPE.namespace("login");
SKYPE.login.Bootstrap=function(f){var e=false;
var a=function(n){f.href=n;
};
var d=function(){return f.href||"";
};
var c=function(){return API.canAutoLogin()&&(parseInt(API.getLogoutReason(),10)===0);
};
var k=function(){return d().match(/network_error/)!==null;
};
var j=function(o){API.log("Loading ClientLogin from cloud URL "+o);
var q=f.search,n=encodeURIComponent($.toJSON(EcsConfig.getEcsResponse())),p="ecs_config="+n;
if(q===""){o+=("?"+p);
}else{o+=q;
o+=("&"+p);
}o+=f.hash;
...
var i=function(){var n;
API.log("Loading ClientLogin from local drive");
n=d().replace("index.html","login.html");
a(n);
};
var b=function(){if(c()){API.setLoginStatusListener(function(n){if(n==0){i();
}});
API.log("Trying autologin");
API.autoLogin();
e=true;
}};
var g=function(){if(typeof window.navigator==="object"&&typeof window.navigator.userAgent==="string"){var o=window.navigator.userAgent,q=o.indexOf("MSIE 6"),p=o.indexOf("MSIE 7"),n=o.indexOf("MSIE 8");
if(n>0){$("html").addClass("lt-ie9");
}else{if(p>0){$("html").addClass("lt-ie9 lt-ie8");
}else{if(q>0){$("html").addClass("lt-ie9 lt-ie8 lt-ie7");
}}}}};
var m=function(){e=false;
API.log("Document loaded, bootstrapping ClientLogin");
g();
b();
if(k()){i();
}};
var l=function(n){if(!e){if(!k()&&n&&n.length>0){j(n);
}else{i();
}}};
var h=function(){i();
};
return{init:m,load:l,onError:h};
};
